...
int master_init(server_config_t *config) {
    ...
    if (logger_init(config->log_file) != 0) {
        fprintf(stderr, "[MASTER] Warning: Failed to initialize logger\n");
    } else {
        printf("[MASTER] Logger initialized: %s\n", config->log_file);
    }

    /* 2) Initialize IPC (shared memory + semaphores) */
